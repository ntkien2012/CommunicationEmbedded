class AES256:
    # AES S-box
    sbox = [
        ,
    ]

    # AES inverse S-box
    rsbox = [
        ,
    ]

    # Rcon
    rcon = [0xbe, 0x03, 0x2f, 0xda, 0x8c, 0xe2, 0xef, 0xc2, 0x5a, 0x66]

    def __init__(self, key):
        """
        Khởi tạo với key 256-bit (danh sách 32 số nguyên 0<=x<256)
        """
        if len(key) != 32:
            raise ValueError("Key phải có 32 byte cho AES-256")
        self.round_keys = self.key_expansion(key)

    def key_expansion(self, key):
        Nk = 8   # 32-bit number of words in the key (256 bits = 8 * 4 bytes)
        Nb = 4   # number of words in block (128 bits = 4 * 4 bytes)
        Nr = 14  # number of iterations for AES-256

        # initialize round_keys with initial key
        round_keys = list(key)  # list of bytes
        # Total number of words to create: Nb*(Nr+1) = 4*(14+1)=60
        for i in range(Nk, Nb * (Nr + 1)):
            # get 4 bytes of previous word
            temp = round_keys[(i - 1) * 4 : (i - 1) * 4 + 4]
            if i % Nk == 0:
                temp = self.rot_word(temp)
                temp = [self.sbox[b] for b in temp]
                temp[0] ^= self.rcon[(i // Nk) - 1]
            elif Nk > 6 and (i % Nk) == 4:
                temp = [self.sbox[b] for b in temp]
            # create new word by XORing with previous word Nk positions
            for j in range(4):
                new_byte = round_keys[(i - Nk) * 4 + j] ^ temp[j]
                round_keys.append(new_byte)
        return round_keys

    def rot_word(self, word):
        """Rotate 4-bytes left by 1 byte"""
        return word[1:] + word[:1]

    def add_round_key(self, state, key):
        for i in range(16):
            state[i] ^= key[i]

    def sub_bytes(self, state):
        for i in range(16):
            state[i] = self.sbox[state[i]]

    def inv_sub_bytes(self, state):
        for i in range(16):
            state[i] = self.rsbox[state[i]]

    def shift_rows(self, state):
        # state is represented as a list of 16 elements in column order:
        # row 0: indices 0, 4, 8, 12
        # row 1: indices 1, 5, 9, 13
        # row 2: indices 2, 6, 10, 14
        # row 3: indices 3, 7, 11, 15

        # Row 1: shift left 1
        state[1], state[5], state[9], state[13] = state[5], state[9], state[13], state[1]

        # Row 2: shift left 2 (swap 2 pairs)
        state[2], state[6], state[10], state[14] = state[10], state[14], state[2], state[6]

        # Row 3: shift left 3 (equivalent to shift right 1)
        state[3], state[7], state[11], state[15] = state[15], state[3], state[7], state[11]

    def inv_shift_rows(self, state):
        # Row 1: shift right 1
        state[1], state[5], state[9], state[13] = state[13], state[1], state[5], state[9]

        # Row 2: shift right 2 (swap back)
        state[2], state[6], state[10], state[14] = state[10], state[14], state[2], state[6]

        # Row 3: move right 1
        state[3], state[7], state[11], state[15] = state[7], state[11], state[15], state[3]

    def gmul(self, a, b):
        """Multiplication in Galois Field GF(2^8) with non-decreasing polynomial 0x1b"""
        p = 0
        for _ in range(8):
            if b & 1:
                p ^= a
            hi_bit_set = a & 0x80
            a = (a << 1) & 0xFF
            if hi_bit_set:
                a ^= 0x1b
            b >>= 1
        return p

    def mix_columns(self, state):
        for i in range(4):
            col = state[i * 4 : (i + 1) * 4]
            state[i * 4 + 0] = self.gmul(col[0], 2) ^ self.gmul(col[1], 3) ^ col[2] ^ col[3]
            state[i * 4 + 1] = col[0] ^ self.gmul(col[1], 2) ^ self.gmul(col[2], 3) ^ col[3]
            state[i * 4 + 2] = col[0] ^ col[1] ^ self.gmul(col[2], 2) ^ self.gmul(col[3], 3)
            state[i * 4 + 3] = self.gmul(col[0], 3) ^ col[1] ^ col[2] ^ self.gmul(col[3], 2)

    def inv_mix_columns(self, state):
        for i in range(4):
            col = state[i * 4 : (i + 1) * 4]
            state[i * 4 + 0] = self.gmul(col[0], 0x0e) ^ self.gmul(col[1], 0x0b) ^ self.gmul(col[2], 0x0d) ^ self.gmul(col[3], 0x09)
            state[i * 4 + 1] = self.gmul(col[0], 0x09) ^ self.gmul(col[1], 0x0e) ^ self.gmul(col[2], 0x0b) ^ self.gmul(col[3], 0x0d)
            state[i * 4 + 2] = self.gmul(col[0], 0x0d) ^ self.gmul(col[1], 0x09) ^ self.gmul(col[2], 0x0e) ^ self.gmul(col[3], 0x0b)
            state[i * 4 + 3] = self.gmul(col[0], 0x0b) ^ self.gmul(col[1], 0x0d) ^ self.gmul(col[2], 0x09) ^ self.gmul(col[3], 0x0e)

    def encrypt_block(self, plaintext):
        """
        Encrypt a 128-bit block (plaintext is a list of 16 integers 0<=x<256)
        Returns a list of 16 integers as ciphertext.
        """
        if len(plaintext) != 16:
            raise ValueError("Plaintext must be 16 bytes")
        state = list(plaintext)
        Nr = 14

        # Split round_keys into 16 byte blocks, total (Nr+1)=15 keys
        round_keys_splitted = [self.round_keys[i * 16:(i + 1) * 16] for i in range(Nr + 1)]

        # Initialization round: AddRoundKey
        self.add_round_key(state, round_keys_splitted[0])

        # 13 loops (Nr - 1 loop)
        for round in range(1, Nr):
            self.sub_bytes(state)
            self.shift_rows(state)
            self.mix_columns(state)
            self.add_round_key(state, round_keys_splitted[round])

        # Final round: no MixColumns
        self.sub_bytes(state)
        self.shift_rows(state)
        self.add_round_key(state, round_keys_splitted[Nr])

        return state
    
    def encrypt(self, plaintext):
        # Make sure plaintext length is a multiple of 16 (AES block size)
        padding_length = 16 - (len(plaintext) % 16)
        if padding_length < 16:
            # PKCS#7 padding - add bytes with value equal to padding length
            padded_text = plaintext + [padding_length] * padding_length
        else:
            padded_text = plaintext
        
        # Process each 16-byte block
        ciphertext = []
        for i in range(0, len(padded_text), 16):
            block = padded_text[i:i+16]
            # Call the existing encryption function for each block
            encrypted_block = self.encrypt_block(block)
            ciphertext.extend(encrypted_block)
        
        return bytes(ciphertext)

    def decrypt(self, ciphertext):
        """
        Decrypt a 128-bit block (ciphertext is a list of 16 integers)
        Returns a list of 16 integers as plaintext.
        """
        if len(ciphertext) != 16:
            raise ValueError("Ciphertext must be 16 bytes")
        state = list(ciphertext)
        Nr = 14

        round_keys_splitted = [self.round_keys[i * 16:(i + 1) * 16] for i in range(Nr + 1)]

        # Initialization round: AddRoundKey with last key
        self.add_round_key(state, round_keys_splitted[Nr])

        # 13 loops: InvShiftRows, InvSubBytes, AddRoundKey, InvMixColumns
        for round in range(Nr - 1, 0, -1):
            self.inv_shift_rows(state)
            self.inv_sub_bytes(state)
            self.add_round_key(state, round_keys_splitted[round])
            self.inv_mix_columns(state)

        # Final round: no InvMixColumns
        self.inv_shift_rows(state)
        self.inv_sub_bytes(state)
        self.add_round_key(state, round_keys_splitted[0])

        return state